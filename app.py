...
def scan_single_stock(symbol: str) -> dict | None:
    """Quét một cổ phiếu: trả về dict kết quả hoặc None nếu lỗi."""
    try:
        ...
        mcdx_now = latest["MCDX"] if pd.notna(latest["MCDX"]) else 0
        if mcdx_now > 0.2:   score += 2
        elif mcdx_now > 0:   score += 1
        else:                 score -= 1

        span_a_s = latest["SpanA"] if pd.notna(latest["SpanA"]) else None
        span_b_s = latest["SpanB"] if pd.notna(latest["SpanB"]) else None
        ...
        if pd.notna(latest["Tenkan"]) and pd.notna(latest["Kijun"]):
            if latest["Tenkan"] > latest["Kijun"]: score += 1
            else:                                  score -= 1

        # Fib scoring
        _is_down  = fib_levels.get("_is_downtrend", True)
        _fh       = fib_levels.get("_high", 0)
        _fl       = fib_levels.get("_low",  0)
        _diff     = fib_levels.get("_diff", _fh - _fl)
        cn        = latest["close"]
        fm        = _diff * 0.03
        if _is_down:
            f618 = _fh - 0.618 * _diff
            f650 = _fh - 0.650 * _diff
            f786 = _fh - 0.786 * _diff
            f382 = _fh - 0.382 * _diff
            if f650 <= cn <= f618:            score += 3
            elif abs(cn - f618) <= fm:         score += 2
            elif f786 - fm <= cn <= f786 + fm: score += 2
            elif cn < f786:                    score -= 1
        else:
            f618 = _fl + 0.618 * _diff
            f382 = _fl + 0.382 * _diff
            f50  = _fl + 0.500 * _diff
            if cn > f618:       score += 2
            elif cn > f50:      score += 1
        ...
        _is_down2 = fib_levels.get("_is_downtrend", True)
        _fh2      = fib_levels.get("_high", 0)
        _fl2      = fib_levels.get("_low",  0)
        _diff2    = fib_levels.get("_diff", _fh2 - _fl2)
        if _is_down2:
            _f618_z = _fh2 - 0.618 * _diff2
            _f650_z = _fh2 - 0.650 * _diff2
            _f786_z = _fh2 - 0.786 * _diff2
            _f382_z = _fh2 - 0.382 * _diff2
            ...
            _f236_z = _fl2 + 0.236 * _diff2
            _f382_z = _fl2 + 0.382 * _diff2
        ...
        return {
            ...
            "rsi":            rsi_now,
            "mcdx":           mcdx_now,
            "macd_bull":      bool(pd.notna(latest["MACD"]) and pd.notna(latest["MACD_Signal"]) and latest["MACD"] > latest["MACD_Signal"]),
            "sm_phase":       sm_result["phase"],
            "sm_icon":        sm_result["icon"],
            "sm_color":       sm_result["color"],
            "sm_score":       sm_result["score"],
            "fib_zone":       fib_zone,
            "score":          score,
            "max_score":      max_score,
            "recommendation": recommendation,
            "obv_slope":      sm_result.get("obv_slope", 0),
            "cmf":            sm_result.get("cmf", 0),
...
def _phase_order(phase: str) -> int:
    order = {"GOM HÀNG": 0, "ĐẨY GIÁ (MARKUP)": 1,
             "TRUNG TÍNH / QUAN SÁT": 2, "XẢ HÀNG (DISTRIBUTION)": 3, "ĐÈ GIÁ (MARKDOWN)": 4}
    return order.get(phase, 5)
...
with _tab2:
    ...
    with col_f1:
        filter_phase = st.multiselect(
            "Lọc theo pha Smart Money:",
            ["GOM HÀNG", "ĐẨY GIÁ (MARKUP)", "TRUNG TÍNH / QUAN SÁT",
             "XẢ HÀNG (DISTRIBUTION)", "ĐÈ GIÁ (MARKDOWN)"],
            default=[],
            placeholder="Tất cả pha",
        )
    with col_f2:
        filter_rec = st.multiselect(
            "Lọc theo khuyến nghị:",
            ["✅ MUA", "⏳ THEO DÕI", "🚫 TRÁNH/BÁN"],
            default=[],
            placeholder="Tất cả khuyến nghị",
        )
    with col_f3:
        filter_fib = st.multiselect(
            "Lọc theo vùng Fibonacci:",
            ["⭐ Golden Pocket", "✅ Gần đáy (tích lũy)", "⚠️ Hỗ trợ sâu (65-78.6%)",
             "📍 Vùng hỗ trợ (38-61.8%)", "📍 Phục hồi sớm"],
            default=[],
            placeholder="Tất cả vùng Fib",
        )
    ...
    if st.button("🚀 Chạy Quét Thị Trường", type="primary", key="scan_btn"):
        ...
        filtered = results
        if filter_phase:
            filtered = [r for r in filtered if r["sm_phase"] in filter_phase]
        if filter_rec:
            filtered = [r for r in filtered if r["recommendation"] in filter_rec]
        if filter_fib:
            filtered = [r for r in filtered if any(fz in r["fib_zone"] for fz in filter_fib)]

        # ── Summary stats ─────────────────────────────────────────────────
        st.markdown("---")
        phase_counts = {}
        for r in results:
            phase_counts[r["sm_phase"]] = phase_counts.get(r["sm_phase"], 0) + 1

        sum_cols = st.columns(5)
        phase_defs = [
            ("GOM HÀNG",            "🏦", "#00C853"),
            ("ĐẨY GIÁ (MARKUP)",    "🚀", "#40C4FF"),
            ("TRUNG TÍNH / QUAN SÁT","🔍","#FFD740"),
            ("XẢ HÀNG (DISTRIBUTION)","📤","#FF6D00"),
            ("ĐÈ GIÁ (MARKDOWN)",   "📉", "#FF1744"),
        ]
        for idx, (ph, ic, co) in enumerate(phase_defs):
            ...
            sum_cols[idx].markdown(
                ...
                f"<div style='font-size:0.75rem; color:#aaa;'>{ph}</div>"
                f"</div>",
                unsafe_allow_html=True,
            )
        ...
        m1.metric("✅ Khuyến nghị MUA",         buy_cnt,   f"/ {len(results)} mã")
        m2.metric("⏳ Theo dõi",                 watch_cnt, f"/ {len(results)} mã")
        m3.metric("🚫 Tránh/Bán",               avoid_cnt, f"/ {len(results)} mã")
        m4.metric("⭐ Vùng Fib hấp dẫn",        gp_cnt,    f"/ {len(results)} mã")

        # ── Per-phase grouped display ──────────────────────────────────────
        st.markdown("---")
        st.markdown(f"### 📋 Kết quả quét — {len(filtered)}/{len(results)} mã (sau bộ lọc)")

        PHASE_META = {
            "GOM HÀNG":              ("🏦", "#00C853", "rgba(0,200,83,0.10)",
                                       "Tiền lớn đang bí mật tích lũy. Cơ hội mua sớm trước khi đẩy giá."),
            "ĐẨY GIÁ (MARKUP)":     ("🚀", "#40C4FF", "rgba(64,196,255,0.10)",
                                       "Cá mập đang markup. Có thể theo đà nhưng quản lý rủi ro chặt."),
            "TRUNG TÍNH / QUAN SÁT":("🔍", "#FFD740", "rgba(255,215,64,0.08)",
                                       "Chưa có tín hiệu rõ ràng. Quan sát thêm."),
            "XẢ HÀNG (DISTRIBUTION)":("📤","#FF6D00","rgba(255,109,0,0.10)",
                                        "Tiền lớn đang phân phối. Cẩn thận, tránh mua đuổi."),
            "ĐÈ GIÁ (MARKDOWN)":    ("📉", "#FF1744", "rgba(255,23,68,0.10)",
                                       "Tay to đang xả mạnh. Không nên mua, xem xét cắt lỗ."),
        }

        # Group filtered results by phase
        from collections import defaultdict
        by_phase = defaultdict(list)
        for r in filtered:
            by_phase[r["sm_phase"]].append(r)
        ...
        for ph_name, (ph_icon, ph_color, ph_bg, ph_desc) in PHASE_META.items():
            ...
            with st.expander(
                f"{ph_icon} {ph_name} — {len(group)} mã", expanded=(ph_name == "GOM HÀNG")
            ):
                st.markdown(
                ...
                rows = []
                for r in group:
                    macd_str = "📈 Bull" if r["macd_bull"] else "📉 Bear"
                    rr = "✅" if r["recommendation"] == "✅ MUA" else ("⏳" if r["recommendation"] == "⏳ THEO DÕI" else "🚫")
                    rows.append({
                        "Mã":           r["symbol"],
                        "Giá (đ)":      f"{r['close']:,.0f}",
                        "% 1P":         f"{r['pct_chg']:+.2f}%",
                        ...
                        "Vol/MA":       f"x{r['vol_ratio']:.2f}",
                        "Điểm SM":      f"{r['sm_score']:+.1f}",
                        "Vùng Fib":     r["fib_zone"],
                        "KN Tổng hợp":  r["recommendation"],
                    })
                df_disp = pd.DataFrame(rows)
                st.dataframe(df_disp, hide_index=True, use_container_width=True)

                # Card view for top picks (MUA + phase bullish)
                ...
                if top_picks and ph_name in ("GOM HÀNG", "ĐẨY GIÁ (MARKUP)"):
                    ...
                    for ci, r in enumerate(top_picks[:8]):
                        with card_cols[ci % 4]:
                            ...
                            rsi_note = "Quá bán 🟢" if r["rsi"] < 35 else ("Quá mua 🔴" if r["rsi"] > 70 else "Trung tính")
                            ...
                            st.markdown(
                                f"""<div style="border:1px solid {ph_color}55; border-top:3px solid {ph_color};
                                    border-radius:10px; padding:14px 12px; background:{ph_bg};
                                    margin-bottom:8px;">
  ...
  <div style="font-size:0.78rem; color:#bbb; line-height:1.6;">
    RSI: <b style="color:#FF9800;">{r['rsi']:.1f}</b> — {rsi_note}<br>
    MACD: {'📈 Bull' if r['macd_bull'] else '📉 Bear'} &nbsp;·&nbsp; MCDX: {r['mcdx']:.3f}<br>
  ...
  <div style="margin-top:8px; font-size:0.82rem; font-weight:700; color:#00C853;">
    Điểm: {r['score']}/{r['max_score']} &nbsp;|&nbsp; {r['recommendation']}
        ...
        # ── Vùng mua hợp lý (Golden Pocket / Near Base) ─────────────────
        st.markdown("---")
        ...
        st.markdown(f"### 🛒 Danh Sách Cổ Phiếu Về Vùng Mua Hợp Lý — {len(vung_mua)} mã")
        st.caption(
            "Cổ phiếu hiện đang trong/gần vùng Fibonacci hỗ trợ quan trọng "
            "(Golden Pocket 61.8–65%, Gần đáy, Hỗ trợ sâu 65–78.6%)"
        )
        if vung_mua:
            buy_rows = []
            for r in vung_mua:
        ...
        if danger_list:
            ...
            st.caption("Các mã có dấu hiệu tiền lớn đang thoát hàng. Cần thận trọng hoặc cân nhắc cắt lỗ.")
            danger_rows = []
        ...
        st.info(
            ...
            "Hệ thống sẽ tự động phân tích từng mã và phân loại theo hành vi tiền lớn:\n"
            "- 🏦 **Gom hàng** — Tiền lớn đang tích lũy bí mật\n"
            "- 🚀 **Đẩy giá** — Cá mập đang markup sau khi gom xong\n"
            "- 📤 **Xả hàng** — Phân phối đỉnh cho nhà đầu tư nhỏ lẻ\n"
            "- 📉 **Đè giá** — Tay to đang bán tháo mạnh\n"
            "- 🛒 **Vùng mua hợp lý** — Giá về Fibonacci hỗ trợ (Golden Pocket, đáy...)"
        )
